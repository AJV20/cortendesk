...
it('rejects a duplicate name and an out-of-range numeric option', function () {
    ...
    Livewire::actingAs($admin)
        ->test(StrategyList::class)
        ->call('create')
        ->set('formName', 'Tampered choices')
        ->set('formOptions.enable-camera', 'maybe')
        ->set('formOptions.verification-method', 'unsupported')
        ->call('save')
        ->assertHasErrors(['formOptions.enable-camera', 'formOptions.verification-method']);

    Livewire::actingAs($admin)
        ->test(StrategyList::class)
        ->call('create')
        ->set('formName', 'Oversized text')
        ->set('formOptions.whitelist', str_repeat('1', 4097))
        ->call('save')
        ->assertHasErrors('formOptions.whitelist');
...
it('keeps exactly one default when the editor marks a second one', function () {
    ...
    $displacedRevision = StrategyRevision::where('strategy_id', $first->id)->latest('revision')->firstOrFail();
    expect($first->fresh()->is_default)->toBeFalse()
        ->and(Strategy::where('is_default', true)->pluck('name')->all())->toBe(['Second'])
        ->and($displacedRevision->snapshot['is_default'])->toBeFalse()
        ->and($first->fresh()->active_revision_id)->not->toBeNull();
});

it('previews an enabled toggle before it re-resolves devices', function () {
    ...
    $component = Livewire::actingAs(uiAdmin())
        ->test(StrategyList::class)
        ->call('toggleEnabled', $strategy->id)
        ->assertSet('previewing', true)
        ->assertSet('impactPreview.affected_count', 1);

    expect($strategy->fresh()->enabled)->toBeTrue()
        ->and($device->fresh()->strategy_id_resolved)->toBe($strategy->id);
    ...
    expect($strategy->fresh()->enabled)->toBeFalse()
        ...
        ->and(ConsoleAudit::where('action', 'strategy.update')->exists())->toBeTrue()
        ->and(StrategyRevision::where('strategy_id', $strategy->id)->latest('revision')->value('affected_devices'))->toBe(1);
});
