...
class StrategyAssignmentImpact
{
    ...
    public static function preview(Strategy $strategy, array $deviceIds, array $userIds, array $groupIds): array
    {
        ...
        hash_update($hash, json_encode([
            ...
            'strategy_updated_at' => $strategy->updated_at?->toJSON(),
            'desired' => $desired,
        ], JSON_THROW_ON_ERROR));
        ...
        $names = Strategy::query()->pluck('name', 'id')->all();
        $defaultId = Strategy::query()->where('enabled', true)->where('is_default', true)->value('id');
        $affectedCount = 0;
        $affectedDevices = [];
        ...
        Device::query()
            ...
            ->chunkById(500, function (Collection $devices) use (
                $strategy, $desiredLookup, $enabled, $names, $defaultId, $hash,
                &$affectedCount, &$affectedDevices,
            ): void {
