...
class StrategyRevision extends Model
{
    ...
    {
        return $this->hasMany(StrategyRollout::class);
    }
    ...
    public static function captureSnapshot(
        ...
        return DB::transaction(function () use ($strategy, $snapshot, $userId, $changeNote, $affectedDevices): self {
            ...
            // Serialise revision-number allocation even when no prior revision
            // exists (locking MAX() alone cannot lock an empty result set).
            ...
            $next = (int) static::query()->where('strategy_id', $strategy->id)->max('revision') + 1;
